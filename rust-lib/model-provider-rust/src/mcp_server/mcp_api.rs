...
pub fn build_json_rpc_request(
    ...
    let id: Value = serde_json::from_str(&id_json)
        .map_err(|e| format!("Invalid id JSON: {}", e))?;
...
pub fn summarize_server_tools(tools_json: String) -> Result<String, String> {
    ...
    let enabled: Vec<&str> = tools.iter().filter(|t| t.enabled).map(|t| t.name.as_str()).collect();
...
pub fn parse_mcp_import_json(raw_json: String, is_desktop: bool) -> Result<String, String> {
    ...
    if let Some(servers_map) = parsed.get("mcpServers").and_then(|v| v.as_object()) {
        for (id, cfg) in servers_map {
            let cfg = match cfg.as_object() { Some(o) => o, None => continue };
            let type_lower = cfg.get("type").and_then(|t| t.as_str()).unwrap_or("").to_lowercase();
            if type_lower == "inmemory" {
                builtin_seen = true;
                builtin_enabled = cfg.get("isActive").and_then(|v| v.as_bool()).unwrap_or(true);
                continue;
            }
            ...
            if has_stdio_shape {
                ...
                let enabled = cfg.get("isActive").and_then(|v| v.as_bool()).unwrap_or(true);
                let name = cfg.get("name").and_then(|v| v.as_str()).unwrap_or(id).to_string();
                ...
                let args: Vec<String> = cfg.get("args").and_then(|v| v.as_array())
                    ...
                    .unwrap_or_default();
                let env: std::collections::HashMap<String, String> = cfg.get("env").and_then(|v| v.as_object())
                    .map(|o| o.iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect())
                    .unwrap_or_default();
                let wd = cfg.get("workingDirectory").and_then(|v| v.as_str()).map(String::from);
                let name_final = if name.trim().is_empty() { id.clone() } else { name };
            ...
            // SSE/HTTP
            let transport = if type_lower.contains("http") { McpTransportType::Http } else { McpTransportType::Sse };
            let enabled = cfg.get("isActive").and_then(|v| v.as_bool()).unwrap_or(true);
            let name = cfg.get("name").and_then(|v| v.as_str()).unwrap_or(id).to_string();
            let url = match cfg.get("baseUrl").and_then(|v| v.as_str()).map(|s| s.trim().to_string()) {
                Some(u) if !u.is_empty() => u,
                _ => continue,
            };
            let headers: std::collections::HashMap<String, String> = cfg.get("headers").and_then(|v| v.as_object())
                .map(|o| o.iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect())
                .unwrap_or_default();
            let name_final = if name.trim().is_empty() { id.clone() } else { name };
...
pub fn build_list_tools_request(id_json: String) -> Result<String, String> {
    build_json_rpc_request(id_json, crate::mcp_server::protocol::methods::LIST_TOOLS.to_string(), None)
}
...
pub fn build_list_resources_request(id_json: String) -> Result<String, String> {
    build_json_rpc_request(id_json, crate::mcp_server::protocol::methods::LIST_RESOURCES.to_string(), None)
}
...
pub fn build_list_prompts_request(id_json: String) -> Result<String, String> {
    build_json_rpc_request(id_json, crate::mcp_server::protocol::methods::LIST_PROMPTS.to_string(), None)
}
...
pub fn build_health_check_request(id_json: String) -> Result<String, String> {
    build_json_rpc_request(id_json, crate::mcp_server::protocol::methods::HEALTH_CHECK.to_string(), None)
}
...
pub fn json_rpc_error_details(code: i32) -> String {
    let (name, default_msg) = match code {
        ...
        crate::mcp_server::protocol::ERROR_INVALID_REQUEST => ("InvalidRequest", "The JSON sent is not a valid Request object"),
        crate::mcp_server::protocol::ERROR_METHOD_NOT_FOUND => ("MethodNotFound", "The method does not exist / is not available"),
        crate::mcp_server::protocol::ERROR_INVALID_PARAMS => ("InvalidParams", "Invalid method parameter(s)"),
        crate::mcp_server::protocol::ERROR_INTERNAL => ("InternalError", "Internal JSON-RPC error"),
        crate::mcp_server::protocol::ERROR_RESOURCE_NOT_FOUND => ("ResourceNotFound", "Resource not found"),
        crate::mcp_server::protocol::ERROR_TOOL_NOT_FOUND => ("ToolNotFound", "Tool not found"),
        crate::mcp_server::protocol::ERROR_TOOL_EXECUTION_FAILED => ("ToolExecutionFailed", "Tool execution failed"),
        crate::mcp_server::protocol::ERROR_PROMPT_NOT_FOUND => ("PromptNotFound", "Prompt not found"),
        _ => ("Unknown", ""),
    };
