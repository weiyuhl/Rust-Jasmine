...
pub fn export_servers_as_ui_json(
    ...
    for s in servers {
        ...
        entry.insert("name".to_string(), serde_json::Value::String(s.name.clone()));
        entry.insert(
            "type".to_string(),
            serde_json::Value::String(match s.transport {
        ...
        entry.insert("description".to_string(), serde_json::Value::String(String::new()));
        ...
        if s.transport != McpTransportType::Stdio && s.transport != McpTransportType::InMemory {
            entry.insert("baseUrl".to_string(), serde_json::Value::String(s.url.clone()));
            if !s.headers.is_empty() {
                let h: serde_json::Map<String, serde_json::Value> = s.headers.iter()
        ...
        if s.transport == McpTransportType::Stdio {
            ...
            if !s.args.is_empty() {
                let a: Vec<serde_json::Value> = s.args.iter().map(|x| serde_json::Value::String(x.clone())).collect();
                entry.insert("args".to_string(), serde_json::Value::Array(a));
            }
            if !s.env.is_empty() {
                let e: serde_json::Map<String, serde_json::Value> = s.env.iter()
                    .map(|(k, v)| (k.clone(), serde_json::Value::String(v.clone())))
                    .collect();
            ...
            if let Some(ref wd) = s.working_directory {
                if !wd.is_empty() {
                    entry.insert("workingDirectory".to_string(), serde_json::Value::String(wd.clone()));
                }
            }
        }
