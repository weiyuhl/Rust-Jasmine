...
pub fn normalize_args_for_tool(
    schema: Option<&Value>,
    args: &Value,
) -> Value {
    ...
    let cloned: Value = serde_json::from_str(&serde_json::to_string(args).unwrap()).unwrap_or_else(|_| args.clone());
...
fn normalize_by_schema(value: &Value, schema: &Value, property_name: Option<&str>) -> Option<Value> {
    ...
    if !unions.is_empty() {
        ...
        if let (Value::String(_), Some("sources")) = (value, property_name) {
            let obj_branch = unions.iter().find(|m| {
                ...
                types.contains(&"object".to_string()) && m.get("properties").and_then(|p| p.get("type")).is_some()
            });
            if let Some(branch) = obj_branch {
                return normalize_by_schema(&json!({"type": value.as_str().unwrap_or("")}), branch, property_name);
            }
        }
        // Try each branch
    ...
    if declared_types.contains(&"object".to_string()) {
        ...
        if let Value::Object(input) = value {
            ...
            for (key, prop_schema) in &props {
                ...
                if let Some(val) = v {
                    out.insert(key.clone(), normalize_by_schema(&val, prop_schema, Some(key)).unwrap_or(val));
                }
            }
        }
    ...
    if declared_types.contains(&"array".to_string()) {
        ...
        for item in &list {
            ...
            if property_name == Some("sources") && item.is_string() && item_types.contains(&"object".to_string()) {
                if items.get("properties").and_then(|p| p.get("type")).is_some() {
    ...
    if declared_types.contains(&"boolean".to_string()) {
        match value {
            ...
            Value::String(s) => {
                ...
                if lower == "true" || lower == "1" || lower == "yes" { return Some(json!(true)); }
                if lower == "false" || lower == "0" || lower == "no" { return Some(json!(false)); }
            }
            _ => {}
        }
    ...
    if declared_types.contains(&"integer".to_string()) {
        match value {
            Value::Number(n) => {
                if let Some(i) = n.as_i64() { return Some(json!(i)); }
                if let Some(f) = n.as_f64() { return Some(json!(f as i64)); }
            }
            Value::String(s) => {
                if let Ok(i) = s.parse::<i64>() { return Some(json!(i)); }
            }
            _ => {}
        }
        return Some(value.clone());
    ...
    if declared_types.contains(&"number".to_string()) {
        match value {
            ...
            Value::String(s) => {
                if let Ok(f) = s.parse::<f64>() { return Some(json!(f)); }
            }
            _ => {}
        }
        return Some(value.clone());
...
fn schema_enum(schema: &Value) -> Vec<Value> {
    schema.get("enum").and_then(|e| e.as_array()).cloned().unwrap_or_default()
}
...
mod tests {
    ...
    fn test_normalize_integer_conversion() {
        ...
        assert_eq!(normalize_by_schema(&json!("42"), &schema, None), Some(json!(42)));
    }

    #[test]
    fn test_normalize_boolean_conversion() {
        let schema = json!({"type": "boolean"});
        assert_eq!(normalize_by_schema(&json!("true"), &schema, None), Some(json!(true)));
        assert_eq!(normalize_by_schema(&json!("false"), &schema, None), Some(json!(false)));
    }
