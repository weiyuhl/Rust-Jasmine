...
impl McpServerManager {
    ...
    pub fn set_tool_needs_approval(&mut self, server_id: &str, tool_name: &str, needs_approval: bool) -> bool {
        let server = match self.servers.get_mut(server_id) {
            Some(s) => s,
            None => return false,
        };
    ...
    pub fn ensure_builtin_fetch_server(&mut self) {
        ...
        self.servers.insert("kelivo_fetch".to_string(), ManagedMcpServer::new(cfg));
    }
    ...
    pub fn validate_config(config: &McpServerConfig) -> Vec<String> {
        ...
        match config.transport {
            ...
            crate::mcp_server::protocol::McpTransportType::Stdio => {
                if config.command.as_ref().map_or(true, |c| c.trim().is_empty()) {
