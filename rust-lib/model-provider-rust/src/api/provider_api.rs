use crate::models::{ModelInfo, ProviderConfig, ProviderKind};
...
#[flutter_rust_bridge::frb]
pub fn default_base_url(provider_id: String) -> String {
    default_base_url_for(&provider_id).to_string()
}

/// 获取供应商的默认 HTTP 请求头（如 OpenRouter 的标识头）。
/// 返回 JSON map。
#[flutter_rust_bridge::frb]
pub fn get_provider_default_headers(
    config_json: String,
) -> Result<String, String> {
    let config: ProviderConfig = serde_json::from_str(&config_json)
        .map_err(|e| format!("Invalid config JSON: {}", e))?;
    let headers = default_provider_headers(&config);
...
pub fn test_connection(
    ...
    let upstream_id = resolve_api_model_id_from_config(&config, &model_id);
    ...
    match config.effective_provider_kind() {
        ...
        ProviderKind::Claude => {
            ClaudeProvider::test_connection(&config, &upstream_id, use_stream)
        }
    }
}
...
/// 返回 Ok(()) 或错误信息字符串。
#[flutter_rust_bridge::frb]
pub fn validate_provider_config(config_json: String) -> Result<(), String> {
    let config: ProviderConfig = serde_json::from_str(&config_json)
        .map_err(|e| format!("Invalid config JSON: {}", e))?;
...
pub fn create_default_config(
    ...
    let base_url = default_base_url_for(&provider_id).to_string();
...
fn resolve_api_model_id_from_config(config: &ProviderConfig, model_id: &str) -> String {
...
fn default_base_url_for(key: &str) -> &'static str {
    ...
    if host.contains("openrouter.ai") {
        headers.insert(
            ...
            serde_json::Value::String(
                "https://github.com/Chevey339/kelivo".to_string(),
            ),
        );
        headers.insert(
